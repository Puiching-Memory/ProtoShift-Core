...
- 初始化新 Godot 原型时，优先调用 ProtoShift MCP 的 `initialize_godot_project` 工具；仅在该工具不可用时，才手动复制 `samples/godot-standard-template/`
...
- 必须先关闭之前的 Godot 实例，再通过 Godot MCP 启动最终版本游戏一次，让人类可以直接检查当前成品
